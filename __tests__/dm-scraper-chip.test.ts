...
import { EmailExtractor } from '@/lib/chips/dm-scraper/email-extractor';
...
describe('DMScraperChip', () => {
  ...
  it('should execute successfully with no leads', async () => {
    ...
    (chip as any).supabase = mockSupabase;
  ...
  it('should extract email from DM and update lead', async () => {
    ...
    (unipileClient.getDirectMessages as jest.Mock).mockResolvedValue(mockMessages);
      ...
      from: jest.fn((table: string) => {
        if (table === 'leads') {
          return {
            select: jest.fn(() => ({
              is: jest.fn(() => ({
                eq: jest.fn(() => Promise.resolve({ data: mockLeads, error: null })),
              })),
            })),
            update: jest.fn(() => {
              updateCalled = true;
